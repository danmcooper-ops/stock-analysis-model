...
to FRED's keyless CSV endpoint when unset) and `MACRO_ANTHROPIC_API_KEY`
(the Claude-generated macro narrative; `ANTHROPIC_API_KEY` is still read as
a fallback). yfinance needs no authentication.
