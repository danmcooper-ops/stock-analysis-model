...
- **Pages worktree:** `~/Desktop/Workspace Folder/.claude/worktrees/pages-live` (branch `pages-live`; used only by the publish routine in Step 8)
- **Snapshots worktree:** `~/Desktop/Workspace Folder/.claude/worktrees/snapshots-data`
- **Python:** `~/Desktop/Workspace Folder/.claude/worktrees/phase-1-api/.venv/bin/python`
- **SSL fix:** set `SSL_CERT_FILE` to the output of `.venv/bin/python -m certifi` before running any Python script
- **Historical snapshots (git branch `data/snapshots`):** `~/Desktop/Workspace Folder/.claude/worktrees/snapshots-data/results_*.json`
...
```
PYTHON="~/Desktop/Workspace Folder/.claude/worktrees/phase-1-api/.venv/bin/python"; SSL_CERT_FILE=$("$PYTHON" -m certifi); export SSL_CERT_FILE; cd "~/Desktop/Workspace Folder"; "$PYTHON" scripts/download_prices.py --output-dir output/prices --max-age-days 2 --tickers $(ls output/prices/*.parquet | sed 's|.*/||;s|\.parquet||') SPY QQQ IWM DIA
```
...
```
PYTHON="~/Desktop/Workspace Folder/.claude/worktrees/phase-1-api/.venv/bin/python"; SSL_CERT_FILE=$("$PYTHON" -m certifi); export SSL_CERT_FILE; cd "~/Desktop/Workspace Folder"; "$PYTHON" scripts/analyze_stock.py --macro --prices-dir output/prices --universe us --min-spread 0 --mcap-min 300e6
```
...
Run as a **single Bash call** (all on one line, semicolons not newlines):
```
PYTHON="~/Desktop/Workspace Folder/.claude/worktrees/phase-1-api/.venv/bin/python"; SSL_CERT_FILE=$("$PYTHON" -m certifi); export SSL_CERT_FILE; cd "~/Desktop/Workspace Folder"; "$PYTHON" scripts/enrich_fdic.py "output/results_$(date +%Y-%m-%d).json"
```
...
Run as a **single Bash call** (all on one line, semicolons not newlines):
```
PYTHON="~/Desktop/Workspace Folder/.claude/worktrees/phase-1-api/.venv/bin/python"; SSL_CERT_FILE=$("$PYTHON" -m certifi); export SSL_CERT_FILE; cd "~/Desktop/Workspace Folder"; "$PYTHON" scripts/enrich_reit.py "output/results_$(date +%Y-%m-%d).json"
```
...
```
PYTHON="~/Desktop/Workspace Folder/.claude/worktrees/phase-1-api/.venv/bin/python"; SSL_CERT_FILE=$("$PYTHON" -m certifi); export SSL_CERT_FILE; cd "~/Desktop/Workspace Folder"; "$PYTHON" scripts/enrich_xbrl.py "output/results_$(date +%Y-%m-%d).json"
```
...
Run as a **single Bash call** (all on one line, semicolons not newlines):
```
PYTHON="~/Desktop/Workspace Folder/.claude/worktrees/phase-1-api/.venv/bin/python"; SSL_CERT_FILE=$("$PYTHON" -m certifi); export SSL_CERT_FILE; cd "~/Desktop/Workspace Folder"; "$PYTHON" scripts/enrich_pipeline.py "output/results_$(date +%Y-%m-%d).json"
```
...
```
PYTHON="~/Desktop/Workspace Folder/.claude/worktrees/phase-1-api/.venv/bin/python"; SSL_CERT_FILE=$("$PYTHON" -m certifi); export SSL_CERT_FILE; cd "~/Desktop/Workspace Folder"; "$PYTHON" scripts/rescore_and_render.py "output/results_$(date +%Y-%m-%d).json"
```
...
```
cp "output/results_$(date +%Y-%m-%d).json" "~/Desktop/Workspace Folder/.claude/worktrees/snapshots-data/"
```
```
git -C "~/Desktop/Workspace Folder/.claude/worktrees/snapshots-data" add "results_$(date +%Y-%m-%d).json"
```
```
git -C "~/Desktop/Workspace Folder/.claude/worktrees/snapshots-data" commit -m "Snapshot: $(date +%Y-%m-%d)"
```
```
git -C "~/Desktop/Workspace Folder/.claude/worktrees/snapshots-data" push origin data/snapshots
```
...
Run as a **single Bash call** (all on one line, semicolons not newlines):
```
PYTHON="~/Desktop/Workspace Folder/.claude/worktrees/phase-1-api/.venv/bin/python"; SSL_CERT_FILE=$("$PYTHON" -m certifi); export SSL_CERT_FILE; cd "~/Desktop/Workspace Folder"; "$PYTHON" scripts/portfolio_report.py --results-dir output/ --prices-dir output/prices
```
...
Run as a **single Bash call** (all on one line, semicolons not newlines):
```
PYTHON="~/Desktop/Workspace Folder/.claude/worktrees/phase-1-api/.venv/bin/python"; SSL_CERT_FILE=$("$PYTHON" -m certifi); export SSL_CERT_FILE; cd "~/Desktop/Workspace Folder"; "$PYTHON" scripts/gate_na_report.py "output/results_$(date +%Y-%m-%d).json"
```
...
Run as a **single Bash call** (all on one line, semicolons not newlines):
```
PYTHON="~/Desktop/Workspace Folder/.claude/worktrees/phase-1-api/.venv/bin/python"; SSL_CERT_FILE=$("$PYTHON" -m certifi); export SSL_CERT_FILE; cd "~/Desktop/Workspace Folder"; "$PYTHON" scripts/validate_ratings.py --snapshot "output/results_$(date +%Y-%m-%d).json" --prices-dir output/prices
```
