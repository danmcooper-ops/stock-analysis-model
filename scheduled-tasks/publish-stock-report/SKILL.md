...
### 2. Copy the five artifacts into the Pages worktree
The HTML lazy-loads `prices.json`, `hist.json`, `details.json`, and the per-ticker shards in `vol/` from its own directory at runtime, so **all five artifacts must be published together**. Run each as a **separate** Bash call:
```
...
```
```
cp "/Users/danmcooper/Desktop/Workspace Folder/output/prices.json" "/Users/danmcooper/Desktop/Workspace Folder/.claude/worktrees/pages-live/docs/prices.json"
```
```
...
```
(The HTML cp source is relative — run from the main repo root. The sidecar paths are absolute so they work from any cwd.)

**The `vol/` sync is not optional, and must NOT be a `cp -R` or a bare `rsync`.** `docs/vol/` holds ~2,300 per-ticker volume shards (~59 MB) that feed the popup price chart's volume strip.
...
`scripts/publish_vol_shards.py` therefore copies **by manifest**: it reads the `vol` key of `output/prices.json` (the only list the client ever requests), copies exactly those `<ticker>.json` files, deletes anything in the destination not on the manifest, and asserts the destination set equals the manifest before returning non-zero on any mismatch. If it exits non-zero, stop and report — do not publish.
...
## Success criteria
- All five artifacts copied and committed to `pages-live` (single amended commit) — `index.html`, the three sidecars, **and the `vol/` shard directory**
- `scripts/publish_vol_shards.py` exited 0 (`docs/vol` matches the `prices.json` manifest exactly). Do **not** substitute a raw `ls output/vol | wc -l` comparison — that directory contains iCloud conflict copies and will not match by design
