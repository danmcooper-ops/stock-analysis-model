...
class TestXbrlBridgeRows:
    ...
    def test_plain_cash_alone_leaves_combined_row_absent(self):
        """No investments tag: the combined row stays NaN (no phantom value)
        and consumers fall back to plain cash through CASH_KEYS order."""
        c = _client_with({
            'Revenues': [_flow(2024, 1000.0)],
            'CashAndCashEquivalentsAtCarryingValue': [_pit(2024, 30.0)],
        })
